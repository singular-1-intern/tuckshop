<#
.SYNOPSIS
  This script is used to run terraform commands in the host space module.
...
Param(
  ...
  [Parameter(Position = 1)][ValidateSet("dev", "test", "stg", "prd", "dr")][string]$Env = "stg",
...
$backendConfig = @{
  ...
  resource_group_name  = "$($ProjectPrefix)-$($Location)-$($Env)-rg-host_space"
  storage_account_name = "$($ProjectPrefix)$($Location)$($Env)sthstfstate"
}
...
# Copy AZURE_* environment variables to ARM_* environment variables.
...
if (![string]::IsNullOrEmpty($Env:AZURE_CLIENT_ID) -and ![string]::IsNullOrEmpty($Env:AZURE_CLIENT_SECRET)) {
  $Env:TF_VAR_service_principal = "{ client_id = `"$($Env:AZURE_CLIENT_ID)`", client_secret = `"$($Env:AZURE_CLIENT_SECRET)`" }"
